...
def generate_sub_briefing(prompt: str, group_name: str = "", language: str = "繁體中文") -> str:
    ...
    Citation codes [[CITE:S1]][[CITE:S2]]... MUST be preserved intact in the output.
    """
...
3. CRITICAL — Citation preservation: Each input article has a citation code [[CITE:S1]], [[CITE:S2]], etc. When you state a fact from that article, append the token right after the sentence, e.g. "...路透社（Reuters）報導。[[CITE:S3]]". NEVER drop, renumber, merge, or invent citation tokens. These are the only link to the source bibliography.
...
5b. CRITICAL — Never write vague expert attributions like 「專家指出」/「學者認為」 without a real person name. If no named expert exists in source, write 「本期無相關專家意見。」 instead of anonymous expert claims.
...
3. CRITICAL — Citation: preserve [[CITE:S1]][[CITE:S2]]... tokens if present in the input. Append the token after the relevant sentence.
3a. STRICT EVIDENCE GATE — Every substantive factual sentence in this section must be supportable by at least one provided [[CITE:Sx]] article. If you cannot support a claim with citation token support, do not write that claim.
...
4a. CRITICAL — Chapter role rules (apply based on the Section field above):
    ...
    • Chapter 七 (七、專家研析): Focus on expert and analyst perspectives, quotes, and assessments from the provided articles. This is the ONLY chapter where expert opinion may appear.
      If expert perspective is used, you MUST name the expert explicitly (no anonymous "專家指出").
      If no named expert exists in source material, write 「本期無相關專家意見。」 instead of anonymous expert claims.
